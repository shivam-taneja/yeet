        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div className="w-full overflow-hidden rounded-[24px] border-2 border-ink bg-butter shadow-[8px_8px_0_var(--color-ink)] p-8 relative">
              ...
              <div className="flex flex-col gap-4">
                <p className="text-sm font-semibold text-ink/80">
                  Building and maintaining Yeet takes time. If this extension
                  saves you time, consider buying me a coffee to help keep the
                  project alive!
                </p>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="w-full overflow-hidden rounded-[24px] border-2 border-ink bg-white shadow-[8px_8px_0_var(--color-ink)] p-8 relative">
              ...
              <p className="text-sm font-semibold text-ink/50 mb-6 -mt-2">
                Choose which types of Threads posts get yeeted across to X.
              </p>

              <div className="flex flex-col divide-y-2 divide-ink/10">
                <SettingRow
                  label="Mention Posts"
                  description={`When composing from someone's profile. Note: their @handle may differ on X.`}
                  checked={settings.threadsContexts["mention-post"]}
                  onChange={() => handleThreadsContext("mention-post")}
                />

                <SettingRow
                  label="Quote Posts"
                  description="When you quote someone's post. Cross-posts your comment text only."
                  checked={settings.threadsContexts["quote"]}
                  onChange={() => handleThreadsContext("quote")}
                />

                <SettingRow
                  label="Replies"
                  description="When you reply to a post. Not recommended — replies often lack context on X."
                  checked={settings.threadsContexts["reply"]}
                  onChange={() => handleThreadsContext("reply")}
                />

                <SettingRow
                  label="Thread Continuation"
                  description="Append to an existing X post. Coming soon."
                  checked={false}
                  locked
                  comingSoon
                  onChange={() => {}}
